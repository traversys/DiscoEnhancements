document.addEventListener('DOMContentLoaded', function() {
  ...
  var ta = document.getElementById('taxonomy-names');
  ...
  ch.addEventListener("change", function() {
    chrome.storage.local.set({
      header_color: ch.checked
    });
  });

  sh.addEventListener("change", function() {
    chrome.storage.local.set({
      subtle_header: sh.checked
    });
  });
  ...
  // Reload page if the box gets unchecked
  document.getElementById("taxonomy-names").onclick = function() {
    chrome.storage.local.get("taxonomy_attrs", function(data) {
      chrome.tabs.query({
        active: true,
        currentWindow: true
        ...
        if (data["taxonomy_attrs"]) {
          // I have no idea why this test works on TRUE
          // If the box is unchecked, it will reload
  ...
  // End of Global Listener Function
